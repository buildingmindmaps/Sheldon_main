...
export function HowItWorksSection() {
  const steps = [{
    ...
    visual: <div className="relative w-full h-full flex items-center justify-center">
          <div className="w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center">
            <div className="w-20 h-20 rounded-full bg-gradient-to-r from-blue-400 to-indigo-500 flex items-center justify-center">
              <motion.div className="w-10 h-10 rounded-full bg-white flex items-center justify-center" animate={{
            scale: [1, 1.1, 1]
          }} transition={{
            duration: 2,
            repeat: Infinity,
        ...
        background: ["radial-gradient(circle at center, rgba(79,70,229,0.1) 0%, transparent 50%)", "radial-gradient(circle at center, rgba(79,70,229,0.2) 0%, transparent 70%)", "radial-gradient(circle at center, rgba(79,70,229,0.1) 0%, transparent 50%)"]
      }} transition={{
        duration: 4,
        repeat: Infinity,
        ease: "easeInOut"
    ...
    visual: <div className="relative w-full h-full flex items-center justify-center">
          ...
          <motion.div className="absolute top-1 right-3" animate={{
        y: [0, -5, 0]
      }} transition={{
        duration: 2,
        repeat: Infinity,
  ...
  return <section className="relative py-24 px-4 sm:px-6 lg:px-12 max-w-7xl mx-auto bg-gray-50 rounded-3xl overflow-hidden">
      ...
      <motion.div className="absolute w-80 h-80 bg-gradient-to-r from-blue-300 to-indigo-300 rounded-full blur-3xl opacity-20 -z-10" animate={{
      x: [0, -70, 0],
      y: [0, 50, 0]
    }} transition={{
      duration: 20,
      ...
      <div className="relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          {steps.map((step, index) => <motion.div key={index} className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 relative z-10 overflow-hidden hover:shadow-lg transition-all duration-300" initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
      ...
      <motion.div className="mt-16 text-center bg-white p-8 rounded-xl shadow-sm border border-gray-100 relative overflow-hidden" initial={{
      opacity: 0,
      y: 20
    }} whileInView={{
      opacity: 1,
