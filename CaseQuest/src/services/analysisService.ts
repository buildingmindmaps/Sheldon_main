...
export const generateFrameworkAnalysis = async (
  ...
  caseFacts: string[],
  caseConversation: string // Add caseFacts parameter
...
<Section 7: CASE DATA>
--> Case Statement: ${caseStatement}
--> Case Facts: ${caseFacts.join('- ')}
--> Case Conversation: ${caseConversation}
...
export const generateReviewScores = async (
  questions: any[] = [], // ✅ Default to empty array
  frameworkText: string,
  timeElapsed: number,
  caseStatement: string,
  caseFacts: string[],
  caseConversation: string,
  conversation: ConversationMessage[] = []  // ✅ Default to empty array
  ...
  try {
    const prompt = `
      ...
      ${Array.isArray(questions) ? questions.map(q => `- ${q.question} (Feedback: ${q.feedback})`).join('\n') : ''}
      
      Conversation History:
      ${Array.isArray(conversation) ? conversation.map(msg => `${msg.sender}: ${msg.message}`).join('\n') : ''}
